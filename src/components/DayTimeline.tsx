"use client"
import { Sunrise, Coffee, TreePine, Utensils, Waves, Star, Moon } from "lucide-react";
...
const timelineEvents = [
  {
    ...
    description: "Wake up naturally to golden sunlight filtering through trees. Enjoy fresh mountain tea on your private deck.",
    icon: Sunrise,
  ...
  },
  {
    ...
    description: "Discover hidden waterfalls and exotic wildlife on guided forest trails with our expert naturalists.",
    icon: TreePine,
  ...
  },
  {
    ...
    description: "Connect with nature through mindful movement and breathing exercises in our forest yoga pavilion.",
    icon: Coffee,
    image: "https://images.unsplash.com/photo-1506629905607-ce19c21c7d93?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80",
    color: "bg-gradient-to-r from-blue-400 to-cyan-500"
  },
  {
    time: "1:00 PM",
    title: "Organic lunch with local ingredients",
    description: "Feast on farm-to-table cuisine featuring organic vegetables and herbs grown in our garden.",
    icon: Utensils,
  ...
  },
  {
    ...
    description: "Paddle through crystal-clear waters surrounded by pristine mountains and spot local wildlife.",
    icon: Waves,
  ...
  },
  {
    ...
    description: "Learn constellations while enjoying s'mores and warm cocoa around our central fire pit.",
    icon: Star,
  ...
  },
  {
    ...
    description: "Drift off in premium comfort with the gentle sounds of nature as your lullaby.",
    icon: Moon,
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16 animate-slide-up">
          ...
          <p className="text-xl text-stone/80 max-w-3xl mx-auto font-poppins">
            Experience the perfect balance of adventure and relaxation. Every moment crafted for unforgettable memories.
          </p>
        </div>
        ...
        <div className="hidden lg:block relative mb-16">
          ...
          <div className="space-y-20">
            {timelineEvents.map((event, index) => (
              <div 
                key={index} 
                className={`flex items-center gap-12 animate-fade-in ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}
                style={{animationDelay: `${index * 200}ms`}}
                onMouseEnter={() => setActiveEvent(index)}
              >
                {/* Content */}
                <div className="lg:w-5/12">
                  <div className={`p-8 rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300 hover-lift ${
                    activeEvent === index ? 'bg-white scale-105' : 'bg-white/90'
                  }`}>
                    <div className={`inline-flex items-center space-x-3 px-4 py-2 rounded-full text-white font-semibold mb-4 ${event.color}`}>
                ...
                {/* Timeline Dot */}
                <div className={`w-8 h-8 rounded-full border-4 border-white shadow-lg z-10 transition-all duration-300 ${
                  activeEvent === index ? event.color.replace('bg-gradient-to-r', 'bg-gradient-to-br') + ' scale-125' : 'bg-moss'
                }`}></div>
        ...
        <div className="lg:hidden space-y-8">
          {timelineEvents.map((event, index) => (
            ...
            >
              <div className={`absolute left-0 top-0 w-12 h-12 rounded-full flex items-center justify-center text-white ${event.color}`}>
                <event.icon className="w-6 h-6" />
              </div>
              <div className="bg-white rounded-3xl p-6 shadow-lg hover:shadow-xl transition-shadow duration-300">
                <div className="text-moss font-bold text-lg mb-2">{event.time}</div>
